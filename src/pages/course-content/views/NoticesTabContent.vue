<template>
  <view class="w-full bg-white rounded-lg px-4 py-3 box-border">
    <view v-for="(a, i) in announcement?.dataList ?? []">
      <view class="text-normal text-gray-9">{{ a.title }}</view>
      <view class="text-description mt-1 text-gray-4">{{
        new Date(a.Time).toLocaleDateString()
      }}</view>
      <nut-divider v-if="i < announcement?.dataList.length!" class="text-gray-200 my-3" />
    </view>
  </view>
</template>
<script setup lang="ts">
import { useAnnouncementsQuery } from '@/composables/courses'

const props = defineProps<{
  courseId: number
}>()

const announcementQuery = useAnnouncementsQuery(props.courseId)
const announcement = announcementQuery.data
</script>
